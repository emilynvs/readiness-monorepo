...
export const Main = () => {
  const { tarefas, loading, updateCheck } = useTarefas();
      ...
      {tarefas.map((tarefa) => (
        ...
        >
          ...
          <div className="flex gap-2">
            ...
            <button
              className={`flex items-center justify-center gap-1 border-2 rounded-xl w-23 h-8 text-base font-medium antialiased leading-none transition-colors duration-500 cursor-pointer outline-none ${
                tarefa.concluido
                  ? "bg-green-500 text-white border-green-500 hover:bg-green-600"
                  : "bg-white text-black border-white hover:bg-slate-500 hover:text-white hover:border-transparent"
              }`}
              onClick={() => {
                updateCheck(tarefa.id);
              }}
            >
              <svg
